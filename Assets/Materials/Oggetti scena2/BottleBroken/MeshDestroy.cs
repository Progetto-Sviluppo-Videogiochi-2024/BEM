...
public class MeshDestroy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            // Crea un ray dal punto del mouse nel mondo
            Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);

            // Controlla se il ray colpisce un collider
            if (Physics.Raycast(ray, out RaycastHit hit))
            {
                // Verifica se l'oggetto colpito ha lo stesso script
                if (hit.collider.TryGetComponent<MeshDestroy>(out var meshDestroy))
                {
                    meshDestroy.DestroyMesh(); // Chiama il metodo solo sull'oggetto cliccato
                }
            }
        }
    }
    ...
    private void AddEdge(int subMesh, PartMesh partMesh, Vector3 normal, Vector3 vertex1, Vector3 vertex2, Vector2 uv1, Vector2 uv2)
    {
        if (!edgeSet)
        {
            ...
            edgeVertex = vertex1;
            edgeUV = uv1;
            edgePlane.SetNormalAndPosition(normal, vertex1);
            return;
        }

        if (edgePlane.GetSide(vertex2))
        {
    ...
    public class PartMesh
    {
        ...
        {
            if (_Triangles.Count <= submesh)
            {
                _Triangles.Add(new List<int>());
            }

            var vertIndex = _Verticies.Count;
            ...
            _Triangles[submesh].Add(vertIndex);
            _Triangles[submesh].Add(vertIndex + 1);
            _Triangles[submesh].Add(vertIndex + 2);
        }
        ...
        public void MakeGameobject(MeshDestroy original)
        {
            ...
            // Distruggi il GameObject dopo 5 secondi
